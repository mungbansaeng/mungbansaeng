...
<script>
    ...
    function pickCalendarDate (object) {
        ...
        if ($("#calendarDayList").data("val") == "start") { // 마지막 날짜 클릭
            ...
            let endWeek = new Date(year + "-" + monthVal + "-" + endDayVal).getDay();
            ...
            if ($(".endYear").length == 0) {
                ...
                form.append("<input type='hidden' class='endWeek' name='endWeek' value='" + endWeek + "'>");
            ...
            const calendarDiffDate = new Date(endDate).getTime() - new Date(startDate).getTime();

            let calendarDayCount;
            let calendarMessage;

            if ($(".pickLimitUnit").val() == "d") {

                calendarDayCount = Math.floor(Math.abs(calendarDiffDate / (1000 * 60 * 60 * 24))); // 총 몇일인지 계산

                calendarMessage = (parseInt($(".pickLimitDay").val()) + 1) + "일";

            } else if ($(".pickLimitUnit").val() == "m") {

                calendarDayCount = Math.floor(Math.abs(calendarDiffDate / (1000 * 60 * 60 * 24 * 30))); // 총 몇일인지 계산

                calendarMessage = $(".pickLimitDay").val() + "달";
            ...
            } else if ($(".pickLimitDay").val() > 0 && calendarDayCount > $(".pickLimitDay").val() && ($(".pickLimit").val() == "both" || $(".pickLimit").val() == "max")) {

                alert("최대 " + calendarMessage + "까지만 선택해주세요.");
            ...
            } else if ($(".pickLimitDay").val() > 0 && $(".pickLimitDay").val() > calendarDayCount && ($(".pickLimit").val() == "both" || $(".pickLimit").val() == "min")) {

                alert("최소 " + calendarMessage + "이상 선택해주세요.");
            ...
            let startWeek = new Date(year + "-" + monthVal + "-" + startDayVal).getDay();
            ...
            if ($(".startYear").length == 0) {
                ...
                form.append("<input type='hidden' class='startWeek' name='startWeek' value='" + startWeek + "'>");
